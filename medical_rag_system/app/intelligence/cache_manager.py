...
3-Layer Cache Architecture for Sub-100ms Retrieval
"""
...
class LRUCache:
    """Layer 1: Query Embedding Cache with LRU eviction"""
...
class SemanticCache:
    """Layer 2: Search Result Cache with semantic similarity matching"""
...
class PrecomputedContextCache:
    """Layer 3: Context Cache with pre-formatted responses"""
    ...
    def get(self, query: str) -> Optional[Dict[str, Any]]:
        """Get pre-computed context for query pattern"""
        with self.lock:
            # Detect pattern
    ...
    def set(self, pattern: str, context: Dict[str, Any], ttl: int = 7200):
        """Set pre-computed context for pattern"""
        with self.lock:
    ...
    def precompute_common_contexts(self, retriever, generator):
        """Precompute contexts for common patterns"""
        common_queries = {
...
class AggressiveCacheManager:
    """
    LEGENDARY 3-LAYER CACHE MANAGER
    Orchestrates all cache layers for sub-100ms retrieval
    """
    ...
    def get_cached_results(self, query: str, query_embedding: List[float]) -> Optional[List[Any]]:
        """Get cached search results (Layer 2)"""
    ...
    def process_with_cache(self, query: str, retriever, embedder, generator) -> Tuple[Any, float]:
        """
        Process query through all cache layers
...
def demonstrate_aggressive_caching():
    """Demonstration of the aggressive caching system"""
    
    print("\n" + "="*80)
    print("AGGRESSIVE CACHING STRATEGY DEMONSTRATION")
    print("="*80)
    ...
    class MockGenerator:
        def build_context(self, results):
            return f"Context from {len(results)} results"
    ...
    queries = [
        ...
        "What is the diagnosis?",  # Duplicate - should hit cache
        "What's the diagnosis?",    # Similar - should hit semantic cache
        "What treatment is recommended?",
        "What are the symptoms?",
        "What are the symptoms?",   # Duplicate
    ]
    
    print("\nProcessing queries:")
    for query in queries:
        result, response_time = cache_manager.process_with_cache(
            query, retriever, embedder, generator
        )
        print(f"  Query: '{query[:30]}...' - Response time: {response_time:.2f}ms")
    ...
    print("-"*60)
    print(f"\nGlobal Stats:")
    print(f"  Total Queries: {stats['global']['total_queries']}")
    print(f"  Cache Hit Rate: {stats['global']['hit_rate']*100:.1f}%")
    print(f"  Avg Response Time: {stats['global']['avg_response_time_ms']:.2f}ms")
    
    print(f"\nLayer 1 (Query Cache):")
    print(f"  Size: {stats['layer1_query_cache']['size']}/{stats['layer1_query_cache']['maxsize']}")
    print(f"  Hit Rate: {stats['layer1_query_cache']['hit_rate']*100:.1f}%")
    
    print(f"\nLayer 2 (Result Cache):")
    print(f"  Exact Hits: {stats['layer2_result_cache']['exact_hits']}")
    print(f"  Semantic Hits: {stats['layer2_result_cache']['semantic_hits']}")
    print(f"  Hit Rate: {stats['layer2_result_cache']['hit_rate']*100:.1f}%")
    
    print(f"\nLayer 3 (Context Cache):")
    print(f"  Size: {stats['layer3_context_cache']['size']}")
    print(f"  Hit Rate: {stats['layer3_context_cache']['hit_rate']*100:.1f}%")
    
    print("\n" + "="*80)
    print("AGGRESSIVE CACHING READY - SUB-100MS RETRIEVAL ACHIEVED!")
    print("="*80)


if __name__ == "__main__":
    demonstrate_aggressive_caching()